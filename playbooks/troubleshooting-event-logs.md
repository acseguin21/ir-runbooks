...
High-level workflow for using event logs and performance traces to resolve customer issues. Fits: utilizes troubleshooting tools (event logs, performance traces); complex product troubleshooting.
...
3. **Correlate** — Map errors/warnings to timestamps and user actions; check for known patterns (KB, internal docs).
